...
tfh_repM <- subset(tfh_rep, subset = clonalFrequency != 1)

fidel <- tfh_repM@meta.data %>% 
...
count_per_clone <- tfh_repM@meta.data %>% 
  select(CTaa) %>% 
  tibble::remove_rownames() %>% 
  group_by(CTaa) %>% 
  summarise(n = n()) 

cluster_count <- tfh_repM@meta.data %>% 
  select(CTaa, integratedcluster) %>% 
  remove_rownames() %>% 
...
```{r Getting data for Figure4A_Relative expression of marker genes used to annotate reference data set in each subset in CHMI data set, include = TRUE, echo=TRUE, warning=FALSE}
...
try.5_subset <- try.5 %>%
  filter(alternative_clusters %in% c("Tfh1", "Tfh1_CM", "Tfh2", "Tfh17"))
```
...
ggplot(data = try.5_subset, aes(x=day, y=freq)) +
  ...
  facet_wrap(~alternative_clusters, ncol = 4) + 
  theme_classic() + 
...
file.names <- list.files(path="/working_groups/boylelab/shared/ZulyPava/Tfh_diversity_TCR_4.4/IBSM40", pattern = 'filtered_contig_annotations.csv', recursive = TRUE)
names <- list.files(path='/working_groups/boylelab/shared/ZulyPava/Tfh_diversity_TCR_4.4/IBSM40/VDJ')
...
cluster_orders <- c("Tfh1", "Tfh1_CM", "Tfh2", "Tfh17")
...
ibsm_rep_subC <-  subset(ibsm_rep, subset = alternative_clusters %in% c("Tfh1", "Tfh1_CM", "Tfh2", "Tfh17"))
...
ibsm_rep_sClon <- subset(ibsm_rep, subset = clonalFrequency != 1)
ibsm_rep_sClon <- subset(ibsm_rep_sClon, subset = day == "day16")
```
...
for (i in unique(ibsm_rep_sClon@meta.data$day)){
  fidel <- ibsm_rep_sClon@meta.data %>% 
    filter(., day == i) %>% 
  ...
  count_per_clone <- ibsm_rep_sClon@meta.data %>% 
    filter(., day == i) %>% 
    select(CTaa) %>% 
    tibble::remove_rownames() %>% 
    group_by(CTaa) %>% 
  ...
  cluster_count <- ibsm_rep_sClon@meta.data %>% 
    filter(., day == i) %>% 
    select(CTaa, alternative_clusters) %>% 
    remove_rownames() %>% 
...
ggsave(plot = mds_d16, filename = "/working_groups/boylelab/shared/ZulyPava/Tfh_diversity_finalcode/output/MDSplots_d16_TfhIBSM_ALTERNATIVE_240724.pdf",
...
ggsave(plot = mds_d36, filename = "/working_groups/boylelab/shared/ZulyPava/Tfh_diversity_finalcode/MDSplots_d36_ALTERNATIVE_240724.pdf",
       width = 420, height= 297, units = "mm", device = "pdf" )
...
# 1. Get unique gene lists for each group
Tfh1Genes   <- unique(upset1 %>% filter(celltype == "Tfh1") %>% pull(gene))
Tfh1CMGenes <- unique(upset1 %>% filter(celltype == "Tfh1CM") %>% pull(gene))
Tfh2Genes   <- unique(upset1 %>% filter(celltype == "Tfh2") %>% pull(gene))

# 2. Compute the disjoint sets (Venn regions)

# Triple intersection
T1_T1CM_T2 <- Reduce(intersect, list(Tfh1Genes, Tfh1CMGenes, Tfh2Genes))

# Pairwise only intersections (remove triple)
T1_T1CM <- setdiff(intersect(Tfh1Genes, Tfh1CMGenes), T1_T1CM_T2)
T1_T2   <- setdiff(intersect(Tfh1Genes, Tfh2Genes), T1_T1CM_T2)
T1CM_T2 <- setdiff(intersect(Tfh1CMGenes, Tfh2Genes), T1_T1CM_T2)

# Unique only
T1_only   <- setdiff(Tfh1Genes, union(union(T1_T1CM, T1_T2), T1_T1CM_T2))
T1CM_only <- setdiff(Tfh1CMGenes, union(union(T1_T1CM, T1CM_T2), T1_T1CM_T2))
T2_only   <- setdiff(Tfh2Genes, union(union(T1_T2, T1CM_T2), T1_T1CM_T2))

# 3. Construct named region vector for eulerr
fit1 <- euler(c(
  "Tfh1"               = length(T1_only),
  "Tfh1CM"             = length(T1CM_only),
  "Tfh2"               = length(T2_only),
  "Tfh1&Tfh1CM"        = length(T1_T1CM),
  "Tfh1&Tfh2"          = length(T1_T2),
  "Tfh1CM&Tfh2"        = length(T1CM_T2),
  "Tfh1&Tfh1CM&Tfh2"   = length(T1_T1CM_T2)
))

# Optional: Custom colors
fill_colors <- c("#9e0142", "#fcc5c0", "#8c96c6")  # Adjust if needed

# 4. Plot the Venn diagram with correct disjoint counts
fig5b <- plot(
  fit1,
  quantities = TRUE,
  fill = fill_colors,
...
Tfh.16_c <- subset(Tfh.16, alternative_clusters %in% c("Tfh1","Tfh1_CM", "Tfh2", "Tfh17") )
log.data <- GetAssayData(Tfh.16_c[["RNA"]], layer = "data")
Tfh.16_d <- SetAssayData(Tfh.16_c,
                       layer = "data",
                       new.data = log.data,
                       assay = "RNA")
...
Stacked_VlnPlot(Tfh.16_d, features = c("MKI67", "CD38", "ICOS"), x_lab_rotate = TRUE,
